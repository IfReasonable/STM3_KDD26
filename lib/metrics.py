...
def MAE_torch(pred, true, mask_value=None):
    if mask_value is not None:
        ...
        if mask.any():
            return torch.abs(true[mask] - pred[mask]).mean()
        else:
...
def RMSE_torch(pred, true, mask_value=None):
    if mask_value is not None:
        mask = true > mask_value
        if mask.any():
            return torch.sqrt(((pred[mask] - true[mask]) ** 2).mean())
        else:
            return torch.tensor(0., device=pred.device)
    else:
...
def MAPE_torch(pred, true, mask_value=None):
    if mask_value is not None:
        mask = (true > mask_value)
        if mask.any():
            mape = torch.abs((true[mask] - pred[mask]) / (true[mask] + 1e-3))
            return mape.mean()
        else:
            return torch.tensor(0., device=pred.device)
    else:
        mape = torch.abs((true - pred) / (true + 1e-3))
        return mape.mean()
...
def All_Metrics(pred, true, mask1, mask2):
    ...
    if type(pred) == np.ndarray:
        ...
        mse = MSE_torch(pred, true, mask1).item()
    else:
        raise TypeError
